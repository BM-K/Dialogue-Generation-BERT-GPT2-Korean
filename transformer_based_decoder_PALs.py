...
class ScaledDotProductAttention(nn.Module):
    def __init__(self, args, isPals):
        super(ScaledDotProductAttention, self).__init__()

        if isPals:
            self.d_k = int(args.d_model / 6)
        else:
...
class MultiheadAttention(nn.Module):
    def __init__(self, args, multi_params=None):
        super(MultiheadAttention, self).__init__()
        self.args = args

        if multi_params is not None:
            self.d_k = int(args.d_model / multi_params)  # 768/6 = 128
            self.d_v = int(args.d_model / multi_params)  # 768/6 = 128
            self.n_heads = multi_params
            self.PALS = True
            self.li1 = nn.Linear(multi_params * self.d_v, args.hidden_size_aug)
            hidden_size = args.hidden_size_aug  # 204
        else:
            self.d_k = int(args.d_model / args.n_heads)
            ...
            hidden_size = args.d_model  # 768

        self.W_Q = nn.Linear(hidden_size, self.d_k * self.n_heads)  # init (512 x 64 * 8) or 204 x 128*6
        self.W_K = nn.Linear(hidden_size, self.d_k * self.n_heads)
        self.W_V = nn.Linear(hidden_size, self.d_v * self.n_heads)
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        context, attn = ScaledDotProductAttention(self.args, self.PALS)(q_s, k_s, v_s, attn_mask=None)
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def forward(self, inputs, pals_outputs):
        residual = inputs  # inputs : [batch_size, len_q, d_model]
        output = self.relu(self.conv1(inputs.transpose(1, 2)))
        output = self.conv2(output).transpose(1, 2)
        return self.layer_norm(output + residual + pals_outputs)
...
class BERTPals(nn.Module):
    def __init__(self, args):
        ...
        self.aug_dense = nn.Linear(args.d_model, args.hidden_size_aug)
        self.aug_dense2 = nn.Linear(args.hidden_size_aug, args.d_model)

        # Attention without the final matrix multiply.
        self.attn = MultiheadAttention(args, 6)
        self.hidden_act_fn = gelu

    def forward(self, hidden_states, attention_mask=None):
        hidden_states_aug = self.aug_dense(hidden_states)
        hidden_states_aug = self.attn(hidden_states_aug, hidden_states_aug, hidden_states_aug, attention_mask)
...
class DecoderLayer(nn.Module):
    def __init__(self, args):
        ...
        self.dec_enc_attn = MultiheadAttention(args, None)
        self.pos_ffn = PoswiseFeedForwardNet(args)
        self.layer_norm = nn.LayerNorm(args.d_model)
        self.PALayers = BERTPals(args)

    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        dec_outputs, dec_enc_attn = self.dec_enc_attn(dec_inputs, enc_outputs, enc_outputs, dec_enc_attn_mask)
        pals_outputs = self.PALayers(dec_inputs)

        with torch.no_grad():
            dec_outputs = self.pos_ffn(dec_outputs, pals_outputs)
...
class Transformer_PALs(nn.Module):
    def __init__(self, cache_dir, args):
        super(Transformer_PALs, self).__init__()
