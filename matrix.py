...
def test_time_visual(args, inputs, outputs, targets, bert_tk, gpt_vocab):
    ...
    print("input> ", input_sentence.replace('_', ' '))

    targets = targets[0:args.max_len].squeeze().tolist()
    target_sentence = ""
    for idx, token in enumerate(targets):
        if token == gpt_vocab[gpt_vocab.eos_token]:
            break
        else:
            target_sentence += gpt_vocab.to_tokens(token)
    print("target> ", target_sentence.replace('▁', ' '))
    ...
    for idx, token in enumerate(outputs):
        ...
        if token == gpt_vocab[gpt_vocab.eos_token]:
            ...
            outputs_sentence += gpt_vocab.to_tokens(token)
    print("output> ", outputs_sentence.replace('▁', ' '))